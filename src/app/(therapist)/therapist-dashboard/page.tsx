...
import TherapistLayout from "../TherapistLayout"; // Ensure this component is properly defined

const Dashboard: React.FC = () => {
  const [selectedGraph, setSelectedGraph] = useState("wau");
  ...
  const wauOptions: ChartOptions<"line"> = {
    ...
    scales: {
      ...
      y: { title: { display: true, text: "Active Users" }, beginAtZero: true, max: 20 },
    },
  };
  ...
  const frequencyOptions = {
    ...
    plugins: {
      ...
      tooltip: {
        callbacks: {
          label: (context: TooltipItem<"bar">) => `Frequency: ${context.raw}`,
        },
      },
    },
    scales: {
      x: { title: { display: true, text: "Day of the Week" } },
      y: { title: { display: true, text: "Interactions" }, beginAtZero: true, max: 6 },
    },
  };
  ...
  const fluencyOptions = {
    ...
    plugins: {
      ...
      tooltip: {
        callbacks: {
          label: (context: TooltipItem<"line">) => `Improvement: ${context.raw}%`,
        },
      },
    },
    scales: {
      x: { title: { display: true, text: "Weeks" } },
      y: { title: { display: true, text: "Improvement (%)" }, beginAtZero: true, max: 100 },
    },
  };
  ...
  const speechOptions = {
    ...
    plugins: {
      ...
      tooltip: {
        callbacks: {
          label: (context: TooltipItem<"bar">) => `Instances: ${context.raw}`,
        },
      },
    },
    scales: {
      x: { title: { display: true, text: "Sessions" } },
      y: { title: { display: true, text: "Speech Instances" }, beginAtZero: true, max: 30 },
    },
  };
  ...
  const getTitle = () => {
    switch (selectedGraph) {
      ...
      case "frequency":
        return "Daily patient interactions";
      case "fluency":
        return "Fluency improvement rate";
      case "speech":
        return "Count of speech instances";
      default:
        return "";
    }
  };
  ...
  const renderChart = () => {
    switch (selectedGraph) {
      case "wau":
        return <Line data={wauData} options={wauOptions} />;
      case "frequency":
        return <Bar data={frequencyData} options={frequencyOptions} />;
      case "fluency":
        return <Line data={fluencyData} options={fluencyOptions} />;
      case "speech":
        return <Bar data={speechData} options={speechOptions} />;
      default:
        return null;
    }
  };
      ...
      <div className="flex flex-col items-center min-h-screen bg-white px-8 py-8">
        ...
        <div className="grid grid-cols-3 gap-6 mb-6 w-full max-w-5xl">
          ...
          <div className="bg-green-400 p-6 rounded-lg shadow-lg text-center">
            <p className="font-semibold text-3xl">Module Two</p>
            <p className="text-lg font-light">Estimated Time: 30–40 seconds</p>
            <p className="text-lg font-light">Total time to complete: 30 minutes</p>
          </div>
          <div className="bg-customDarkBlue text-white p-6 rounded-lg shadow-lg text-center">
            <p className="font-semibold text-3xl">Module Three</p>
            <p className="text-lg font-light">Estimated Time: 40 seconds-1 minute</p>
            <p className="text-lg font-light">Total time to complete: 30 minutes</p>
          </div>
        </div>
        <div className="mb-6">
          <label htmlFor="graph-selector" className="mr-2 font-medium">
            Select Graph:
          </label>
          <select
            id="graph-selector"
          ...
          >
            ...
            <option value="frequency">Daily Patient Interactions</option>
            <option value="fluency">Fluency Improvement Rate</option>
            <option value="speech">Speech Instance Count</option>
          </select>
        </div>
        <h2 className="text-xl font-semibold mb-4">{getTitle()}</h2>
        <div className="w-full max-w-3xl">{renderChart()}</div>
      </div>
    </TherapistLayout>
  );
};
